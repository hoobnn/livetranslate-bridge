...
输出选择器会标记可回灌的设备，输入选择器会警告可能形成反馈的回环设备。启用「用原说话人的音色播报」后，有译声输出的方向改用支持实时音色复刻的 `qwen3.5-livetranslate-flash-realtime`：麦克风方向在会话开始时复刻一次，所选应用方向每次回复前更新音色。关闭时使用 `qwen3.8-livetranslate-flash-realtime` 的默认音色。所选应用若是音乐播放器，复刻的是识别到的人声，效果取决于音乐中的人声是否清晰。

App 直接绑定用户选择的 Core Audio 输入和输出，但不会修改 macOS 的全局默认设备，也不会安装 HAL 驱动。

## 断句与延迟

一句话从说完到字幕出现，时间花在三处：本地采集与上行、服务端等一段停顿来确认「这句说完了」、以及模型本身的翻译。只有中间那段是可调的，而它默认是一整秒——服务端不收 `turn_detection` 时就按 1000 毫秒等，这一秒加在每一句上。

「设置 → 翻译 → 断句」把它放出来，默认 400 毫秒：

- **停顿多久算说完**（200–2000 毫秒）。调短，字幕更早出现；调得太短，说话人中途换气会被当成说完，一句话被拆成两半分别翻译，每半都拿不到另一半的上下文——这是真正会让译文出错的地方，而不只是变慢。安静的一对一通话 300–400 毫秒即可；讲话常有停顿思考，或环境嘈杂，就往长了调。
- **人声灵敏度**（服务端 VAD 阈值，默认 0.2）。对方声音小、字幕迟迟不出现就调低；键盘声、环境噪声频繁触发就调高。日志里每秒一行的 dBFS 峰值可以直接对照——低于 1000（约 −30 dBFS）会提示 VAD 不会触发。

两项都在 `session.update` 里发出，会话开始后固定，因此停止当前会话后才生效。日志会打印请求值与服务端回显的实际值，服务端若改写了请求值会单独报一行。
